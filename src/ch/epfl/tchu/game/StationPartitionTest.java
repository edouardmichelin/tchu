...
public class StationPartitionTest {
    ...
    @Test
    void stationPartitionWorksOnLinksJoining() {
        var size = 34;
        ...
        var t1 = connectivity.connected(GEN, ZUR);
        var t2 = connectivity.connected(LUG, YVE);
    ...
    void stationPartitionWorksOnGivenExample() {
        ...
        var t1 = partition.connected(stations.get(5), stations.get(3));
        var t2 = partition.connected(stations.get(6), stations.get(11));
        var t3 = partition.connected(stations.get(13), stations.get(11));
        var t4 = partition.connected(stations.get(9), stations.get(11));
        var f1 = partition.connected(stations.get(0), stations.get(6));
        assertTrue(t1);   // Lausanne - Interlaken
        assertTrue(t2);  // Lucerne - Wassen
        assertTrue(t3); // Zoug - Wassen
        assertTrue(t4);  // Schwyz - Wassen

        assertFalse(f1);  // Berne - Lucerne
    }
    ...
    void stationPartitionWorksOnKnownExample1() {
        ...
        // TestRandomizer.RANDOM_ITERATIONS
        for (int i = 0; i < 1; i++) {
            ...
            var t1 = p.connected(chMap.LOC, chMap.BAL);
            var t2 = p.connected(chMap.BER, chMap.LUC);
            var t3 = p.connected(chMap.ZUR, chMap.ZUR);
            var f1 = p.connected(chMap.BER, chMap.SOL);
            var f2 = p.connected(chMap.LAU, chMap.LUC);
            var f3 = p.connected(chMap.ZUR, chMap.KRE);
            assertTrue(t1);
            assertTrue(t2);
            assertFalse(f1);
            assertFalse(f2);
